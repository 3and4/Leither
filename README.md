...
### 特点
* **多平台适配**
    支持常见操作系统（windows,linux,darwin等），可以运行在服务器、pc、手机、nas、路由等常见的设备上。
* **功能完备**
    系统实现了认证体系、应用体系、文件和数据库系统，域名和网络系统。
    系统提供200多个api接口，可以实现常见的互联网应用。
* **容器协同**
    容器之间可以构建一个去中心化网络
    节点之间也可以互相协同，构建出复杂的大型互联网应用

### 用途
* **大模型的工作台**
    把容器作为工作台提供给大模型，让大模型成为你的员工，所有工作内容和成果都代表你的意志，产权和收益都属于你。
* **个人的数据中心**
    存储和管理用户的私域数据，完全由用户掌控。
* **构建去中心化的互联网**
    破除互联网巨头的垄断，构建用户自主控制的网络生态。信息（包括应用）的生成、存储、发布、展现、传播都是完全去中心化的，各个环节的规则透明，用户都可以自建，没有哪一环是不可替代的。整个网络的特点是：用户构建、用户控制、用户享有。
