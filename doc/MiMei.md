...
<!-->
#### 9.1 关系链模型
人是社会化动物，具体体现在每个人都处在大小不同关系圈中。

这里我们描述两种关系圈的拓扑结构：  
**我的朋友圈**
这种结构是以“我”为中心的  
是和我相关的关系好友，可以通过标签对关系好友进行细分管理。  
好友圈是由“我”负责维护和管理的  
这种关系链模型都可以运行在用户自己的结点上。
可以通过互助的方式让好友节点来帮忙完成路由备份均衡等服务
节点之间的节算是两个点之间的**信用结算**问题。

**我参加的群**
这种结构是以群体为中心，我选择加入了这个群体。
先有群这个组织，然后才有群成员的加入。
...
在现实世界货币是人和人之间的信用结算工具。
在Leither网络中，是通过信用方式进行交换。  
每个节点都可以向第三方进行信用授权，这个授权是允许第三方可以使用我的节点的服务量。
节点和节点之间服务量的差是借方向出错方签发的借条，也可以理解成借方发行的货币。
对于一个群体，发行的货币本质上是成员向群体捐献的劳动凭证，这个凭证可以向成员交换服务
...
### 十、群和共识机制
**共识**
比特币诞生之后，给大家展示了一个通过分布式算法实现共识的机制。
区块链把共识部分进行强化
