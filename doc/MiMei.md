...
+ **弥媒**  
弥媒是Leither系统中最核心的创意，参考基因和弥母的概念创制而成。弥母（Meme）可以简单地理解为文化的基因，是一种可以通过模仿等方式在人与人之间传播的文化或者行为元素的载体。  
...
Leither系统实现了认证体系、应用体系、文件和数据库系统、域名和网络系统。API支持常见四十多种语言。  
...
**7.3.3、Leither补充了Html5缺少的环节**  
Leither补充了Html5构建云应用时缺少的环节， Leither提供用户和安全认证机制，提供了应用体系，提供了云端的文件系统和数据库系统，提供了去中心的域名解析机制，冗错及负载均衡机制。


**7.3.4、Leither非常适合构建云服务**  
Leither非常适合构建云服务。除了能完整构建云应用，系统内的应用和数据资源都是可以流动的弥媒形态，加上负载均衡机制；开发的过程中尽量引导开发者把计算放在前端进行，这样大量减少了对主机的要求，同一台主机保守可以比传统方式高一到两个数量级，两者的效率差可以进行量化计算和直接评测。  
...
我们会优选带宽作为节点间结算的基准单位，其他类型的服务通过系数进行转换。除了上面讲的信用标的，信用标的也可以是现实世界的法币    

**个人信用模型**  
这种信用模型是用户与用户之间的（点对点），信用证和借条
+ 信用证  
信用证可以理解为允许免费使用的服务量，借条可以理解为已免费使用的服务量。  
...
+ **互助**  
用户节点在建立的时候，同时具备了提供服务的能力。可以通过主动向对方提供有益的服务获取信用。包括不限于:数据备份、均衡、路由、检索、中介等服务。
