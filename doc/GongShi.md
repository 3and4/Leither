...
稀疏默克尔树是针对共识信息构建的，保存组织的核心信息，主要是类似区块链中的公共账本信息。   
...
因此我们对整个组织的账户进行分组，把稀疏默克尔树的每若干节点归为一个节点组集中管理，每个节点组在树结构上体现为一个稀疏默克尔树分支。  
节点组划分条件包括但不限于子树节点数目或层高。当前节点组上限是包含不超过256个子节点，最多涵盖64层。    
...
选举的规则是共识的一部分，由组织者设定，可以根据序号加账号产生的20位随机数距离目标分支的距离、质押资产的数量、网速的快慢等多方面综合考虑。
...
**网络心跳**  
为了协调各个网络设备节点之间的操作，系统定期生成一个递增的数据序号。  
这个数据序号相当于同步各节点时钟信号，也相当于数据的版本号。在一个周期内，用户将上个周期新增的数据进行因固化保存。根据网络的层级，每个节点负责稀疏默克尔树的若干层数据处理工作。处理工作相当于传统区块链的打块，会获取相应的系统奖励。  
这个序号是由根级的记账负责人共同协商产生的，然后逐级广播到每个在线的网络设备。  

**时空镜相**  
参考Leveldb，通过流式记录新增变动的数据，我们可以极大的优化写操作，达到存储介值的极限，写操作甚至快于数据库的读操作。另一方面通过键值序号提供了修改的版本号，我们可以通过版本查询到历史记录的值。网络心跳相当生成了leveldb中的数据版本序号，通过版本，我们可以快速记录并标识每一个周期内的变动信息。也可以用这种方式记录多维的默克尔树时空树。  
