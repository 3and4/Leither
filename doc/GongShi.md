...
+ 对接服务  
  这部分是提供Leither与传统互联网进行对接的服务的APP、域名解析、公众号、小程序等。
...
创建之后，组织运行在了创建时候的节点上。分布式的组织网络也开始在这个节点上工作。系统缺省的网络结构是DHT网络，这个网络保存组织的公共信息。  

通过DHT网络可经查询组织的公共信息，可以路由查询成员的节点信息，成员之间可以点对点发送消息。群聊信息属于公共信息，保存存在DHT网络上。  
...
3、运营者用各种方式，包括不限于检索标签，app,公众号，小程序、运营、微信群推广等方式把内容展现给用户。  
4、用户浏览内容，同时展示广告，形成传统互联网中的流量 。  
5、展示页面中有广告方认同的检查机制，流量信息会触发合约。合约会分别把代币从广告方账户支付到整个流程参与者的账户。  

	这个奖励使系统健康成长，对他人有越来越高的价值

+ **挖矿奖励**  
这是传统意义上的挖矿。所有在线的节点都可以参与投票记账和合约服务，在打块的时候发放奖励。
...
Leither的ID是由hash生成，每个ID本质上是20字节，160位。所有的可能账号都可以存放在一个160层的二叉树上。每个ID在树上都有确定的位置。实际应用中因为用到的ID远小于这个叶节点的规模，这颗树是一个稀疏的二叉树。树的根节点有两个子节点0x0、0x1，有四个孙节点0x00、0x01、0x10、0x11，如此类推，账号挂接在第160层。  
...
可以快速的找到一个节点上相关的信息。有利于更简单高效完成后面的时空镜相、分片、心跳、转账、合约等操作。每个账号的账户信息都挂接在这颗树的叶节点上，每个分支根据子节点的hash节点和汇总信息生成本节点的hash。

**网络心跳**  
心跳是定期选举出几个负责的节点，同时产生一个递增的数据序号。缺省心跳频率为每秒一次，一个心跳周期为30分钟。
...
**时空镜相**  
LevelDB中，通过流式记录新增变量可以极大的优化写操作，达到存储介值的极限，甚至快于数据库的读操作。另一方面通过键值序号提供了修改的版本号，我们可以通过版本查询到历史记录的值。网络心跳相当生成了LevelDB中的数据版本序号。通过版本，我们可以快速记录每一个周期内的账务信息。

也可以用这种方式记录多维的默克尔树时空树。因为操作的时候不改变其它节点的树结构，除了修改的部分，前后两个版本的稀疏默克尔树结构是一致的。只需要处理差值便可。
...
+ 其他子组织  
  子组织:每个账户都可以看作是一个160位的数字，所有账户都挂接在默克尔树上，如果参考子网掩码(subnet mask)的做法。从低位到高位逐个取掩码，每个账号同时属于160个子组织（子树）。  
  我们可以通过子组织的概念，对这个问题调整一下：对于任何一个不包含交易方的子组织成员，只需要确定交易双方所在的子组织不存在“无中生有”和“账目平衡”便可 。   

#### 7.3、**隐匿策略**  
**子组织遮蔽**  
如果子组织是一个结算单位，交易信息可以隐匿在一个子组织之内  
其他用户只需要关注这个子组织的总账目是否平衡合法便可。
...
远处的账号只能获取少数顶级分支节点的汇聚信息，不清楚下层的账号信息。
