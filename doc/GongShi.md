...
帐号信息隐匿我们就无法检查交易是否合法，交易方有可能使用了超出他所持量的资金。  

**解决**  
对于交易方之外的群体而言，了解信息的目的是不希望被他人的交易非法伤害，而不是希望知道交易的详细信息。如果能解决了这个问题，群体没必要知道双方的交易细节。
...
对于系统奖励也可以当成组织帐户向用户的转账，这样总帐也是前后不变的。

7.2.3、**安全分析**  
+ 交易双方  
  转入方会严格检查以上三个点，否则金额无法到自己的帐户里。  
...
+ 其他子组织  
  子组织:每个帐户都可以看作是一个160位的数字，所有账户都挂接在默克尔树上，如果参考子网掩码(subnet mask)的做法。从低位到高位逐个取掩码，每个账号同时属于160个子组织（子树）。  
  我们可以通过子组织的概念，对这个问题调整一下：对于任何一个不包含交易方的子组织成员，只需要确定交易双方所在的子组织不存在“无中生有”和“账目平衡”便可 。   

#### 7.3、**隐匿策略**  
**子组织遮蔽**  
如果子组织是一个结算单位，交易信息可以隐匿在一个子组织之内  
其他用户只需要关注这个子组织的总帐目是否平衡合法便可。
...
远处的账号只能获取少数顶级分支节点的汇聚信息，不清楚下层的帐号信息。
既保证了交易信息的安全确认，又保证了信息的隐匿

#### 7.4、**隐匿方案**  
本方案中我们用以下的方式实现信息隐匿。  
**信用背书**  
交易可以指定第三方进行背书，缺省由话事人完成。对于其他分支用户，交易是在分支上进行的。
因为背书人作了交易担保，相关交易第三方不会受损。

**邻居检查**  
交易完成之后，默克尔树发生变动，话事人需要广播变动信息。
因为冗余备份关系，交易方的临近节点会拿到详细信息，对树和交易进行检查。
如果发生异常，则进入争议处理流程。
除了少数话事人和邻近节点，绝大多数用户都获取不到交易的详细信息。
