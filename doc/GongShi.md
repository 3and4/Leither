...
选举的规则是根据序号账号产生的20位随机数距离目标分支的距离，质押资产的数量，网速的快慢综合考虑。
...
上级话事人检查总帐是否平衡  
分支话事人检查所在分支交易的合法性  
所有叶节点同步信息时检查邻近分支是否平衡，附近的帐号是否正常。  

**争议流程和处罚机制**    
所有全网在线的节点都会审核一定范围的交易账单。  
每笔交易都需要有足够的保证金背书足够的时间，以供更多的节点进行审核。
交易是同时多个节点处理，话事人和备份话事人都是随机分配的，避免串通的可能性, 任何一个节点有异议，都会进入争议流程，争议期相关金额会被冻结。全网对出错交易进行验证，根据验证结果进行投票，出错节点的保证金会被没收。
话事人只能处理冻结资金所支持的交易金额，超出冻结资金的交易延时到账，直到交易的安全阀值
